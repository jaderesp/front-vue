...
exports.formatVarsString = async (mensagem, number, id_conta, verifStatus, userLogged) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (mensagem.indexOf(varTexto.protocolo.toString()) != -1) {
            ...
            let { id_ticket } = userLogged

            var status = verifStatus ? verifStatus : 'ABERTO'

            let where_ = {}

            if (!id_ticket) {
                where_ = { 'id_ticket': id_ticket }
            } else {
                where_ = { 'idContato': contato_send.idContato, 'status': status }
            }

            let ticket = await ticketMod.getLastReg(where_);
        ...
        if (mensagem.indexOf(varTexto.atendente.toString()) != -1) {
            ...
            var status = verifStatus ? verifStatus : 'ABERTO';
            let ticket = await ticketMod.getSomeOne({ 'idContato': contato_send.idContato, 'status': status });

            //dados do usuario logado
            let { idusuario } = userLogged
            //se existir usuario relacionado ao ticket então pegar informações do mesmo...
            let idUsuadio_ = ticket.idusuario ? ticket.idusuario : idusuario
            let usuarioAtendente = await usuarioMod.getSomeOne({ 'idusuario': idUsuadio_ });
        ...
        if (mensagem.indexOf(varTexto.setor.toString()) != -1) {
            ...
            if (!ticket) {
                ticket = await ticketMod.getSomeOne({ 'idContato': contato_send.idContato, 'status': status });
            }
